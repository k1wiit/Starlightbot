...
class VoiceChannelCommand(private val plugin: StarlightBot) {
    ...
    fun handle(event: SlashCommandInteractionEvent) {
        ...
        try {
            ...
            val action = guild.createVoiceChannel(channelName)
            if (category != null) action.setParent(category)
            if (userLimit > 0) action.setUserlimit(userLimit)
            
            action.queue({ voiceChannel ->
